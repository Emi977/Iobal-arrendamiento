...
async def listar(propiedad_id: int | None = None, db: AsyncSession = Depends(get_db), me: TokenPayload = Depends(get_current_user)):
    q = select(Mensaje)
    if propiedad_id:
...
async def marcar_leido(id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
